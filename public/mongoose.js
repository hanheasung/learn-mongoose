...
document.querySelectorAll("#user-list tr").forEach((el) => {
  ...
  // 사용자 로딩
    ...
    try {
      ...
      tbody.innerHTML = ""; // 이전에 표시된 내용 삭제

      // 사용자 정보를 순회하며 테이블에 사용자 추가
        ...
        td.textContent = user.name; // 사용자의 이름을 셀에 추가
        row.appendChild(td);

        // 나이를 셀 추가
        td = document.createElement('td');
        td.textContent = user.age; // 사용자의 나이를 셀에 추가
        row.appendChild(td);
      ...
      console.error(err); // 오류가 발생할 경우 콘솔에 오류를 출력
    }
    }
// 댓글 로딩
async function getComment(id) {
    try {
      ...
      // 댓글 정보를 순회하며 테이블에 댓글 추가
        comments.map(function (comment) {
        const row = document.createElement('tr'); // 새로운 행을 생성
        // 댓글 ID를 표시하는 셀 생성하고 콘텐츠 추가
        let td = document.createElement('td');
        td.textContent = comment._id;
        row.appendChild(td);

        // 댓글 작성자를 표시하는 셀을 생성하고 콘텐츠를 추가
        td = document.createElement('td');
  ...
  // 사용자 등록 -> 사용자 등록 폼의 submit 이벤트 처리
    ...
    const name = e.target.username.value; // 입력된 사용자의 이름을 가져옵니다.
    const age = e.target.age.value; // 입력된 사용자의 나이를 가져옵니다.
